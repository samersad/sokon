...
class _EditApartmentState extends State<EditApartment> {
  ...
  Widget _buildMediaSection() {
    ...
    final hasVideo = viewModel.videoFile != null ||
        (viewModel.existingVideoUrl?.isNotEmpty ?? false);
  ...
  Widget _buildMediaPreviewSection() {
    ...
    return _SectionCard(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          if (existingImages.isNotEmpty || viewModel.apartmentImages.isNotEmpty)
            SizedBox(
              height: 104.h,
              child: ListView.separated(
                scrollDirection: Axis.horizontal,
                itemCount: existingImages.length + viewModel.apartmentImages.length,
                separatorBuilder: (context, index) => SizedBox(width: 10.w),
                itemBuilder: (context, index) {
                  if (index < existingImages.length) {
                    return _NetworkPhotoPreviewTile(
                  ...
                  return _PhotoPreviewTile(
                    image: viewModel.apartmentImages[fileIndex],
                    onTap: () => openFullScreenGallery(
                      context,
                      fileIndex,
                      viewModel.apartmentImages,
                    ),
                    onRemove: () => viewModel.removeNewImage(fileIndex),
                  );
                },
              ),
            ),
          if ((existingImages.isNotEmpty || viewModel.apartmentImages.isNotEmpty) &&
              (localVideoPath != null ||
                  (remoteVideoUrl != null && remoteVideoUrl.isNotEmpty)))
            SizedBox(height: 16.h),
                    ...
                    child: _RemoveMediaButton(
                      onTap: () {
                        setState(() {
                          ...
                          viewModel.existingVideoUrl = null;
                        });
                      },
                    ),
                  ),
...
class _NetworkPhotoPreviewTile extends StatelessWidget {
  const _NetworkPhotoPreviewTile({
    required this.url,
    required this.onRemove,
  });
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: SizedBox(
        ...
        child: Icon(
          icon,
          color: _EditApartmentState._accentColor(context),
          size: 18.sp,
        ),
      ),
    );
